...
def payment(request):
    ...
    if not saved_address.is_fully_filled():
        ...
        return redirect(request, "App_Payment:checkout")
    if not request.user.profile.is_fully_filled():
        messages.info(request, "Please Complete Profile Details")
        return redirect("App_Login:profile")

    store_id ='samla61d970e11df85'
    API_Key ='samla61d970e11df85@ssl'
    mypayment = SSLCSession(sslc_is_sandbox=True, sslc_store_id= store_id,
    sslc_store_pass= API_Key)
#this will generate the payment_data such val_id, tran_id used for complete method
    ...
    num_of_item=order_item_count, shipping_method='Courier', product_profile='None')
...
def complete(request):
    if request.method == 'POST' or request.method =='post':
        ...
        if status == 'VALID':
            ...
            #removing the product from the cart after purchased
...
def order_view(request):
    try:
        ...
        messages.warning(request, "You do not have an achive order")
